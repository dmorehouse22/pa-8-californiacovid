...
cases |> 
  ...
  mutate(date = as_date(date),
         ...
         year = year(date)) |> 
  unite(month, year, col = "month_year", sep = " ") |> 
  group_by(month_year) |> 
  summarise(total_cases = sum(cases_daily)) |>  
  ggplot() + 
    geom_col(mapping = aes(x = fct_reorder(month_year, my(month_year)), 
                           y = total_cases)) + 
...
cases |> 
  ...
  labs(x = "Month in 2022", 
       y = "Total COVID Cases", 
       title = "COVID Cases by Month in California in 2022") + 
  ggsave("PA8_figure.png")
```
